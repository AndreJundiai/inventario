...
// Verifica se a opção "Administrador" está presente no array de opções enviadas pelo formulário
if (!in_array("Administrador", $_POST["opcoes"])) {
    // Se a opção "Administrador" não estiver presente, redirecione para alguma página de erro ou faça o tratamento necessário
    header("Location: login.php"); // Redireciona para uma página de erro
    exit();
}
...
<body>
    <form class="container mt-4" action="registro.php" method="post">
        ...
        <select name="opcoes[]" multiple size="2">
